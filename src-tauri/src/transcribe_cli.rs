//! Headless lifelog transcriber, embedded in the `nbp` binary.
//!
//!   nbp transcribe <file>... [--out <path>] [--pause <sec>] [--sidecar <path>]
//!
...
//! a daemon drops files in a folder, this turns each into a timecoded `.txt` that
//! downstream LLM processing consumes.
//!
//! The audio is read via the sidecar's AVFoundation-backed loader, so any format
//! it can decode (WAV, m4a, mov/qta, …) works — not just WAV.
...
/// Entry point for the `transcribe` subcommand. `args` is the full process argv
/// (`argv[0]` = program, `argv[1]` = "transcribe"). Returns the process exit code.
pub fn run(args: &[String]) -> i32 {
    if args.len() < 2 || args[1] != "transcribe" {
        eprintln!(
            "Usage: nbp transcribe <file>... [--out <path>] [--pause <sec>] [--sidecar <path>]"
        );
        return 2;
    }
    ...
    if out.is_some() && inputs.len() > 1 {
        eprintln!(
            "--out takes a single file; for a batch omit it (writes <file>.txt next to each)."
        );
        return 2;
    }
    ...
    for input in &inputs {
        let out_path = match &out {
            ...
            None => PathBuf::from(input).with_extension("txt"),
        };
        match transcribe_one(&sidecar, input, pause, &out_path) {
            Ok(n) => eprintln!(
                "\u{2713} {input} \u{2192} {} ({n} segments)",
                out_path.display()
            ),
            Err(e) => {
                eprintln!("\u{2717} {input}: {e}");
                failures += 1;
            }
        }
    }
...
fn transcribe_one(
    ...
    eprintln!("Transcribing {input} \u{2026}");
    ...
    let start = parse_recorder_start(input);
    let body = render(&parsed, start, input);
...
fn parse_recorder_start(input: &str) -> Option<NaiveDateTime> {
...
fn render(out: &SidecarOut, start: Option<NaiveDateTime>, input: &str) -> String {
...
/// Locate the sidecar: explicit override → env → next to our own exe (prod
/// bundle) → the dev `binaries/` dir → the Swift `.build/release/` output.
fn resolve_sidecar(override_path: Option<&str>) -> Option<PathBuf> {
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            cands.push(dir.join(bundled)); // prod: sidecar next to the app binary
            // dev: src-tauri/target/{debug,release}/nbp → src-tauri/
            if let Some(src_tauri) = dir.parent().and_then(|p| p.parent()) {
                ...
                if let Some(repo) = src_tauri.parent() {
                    cands.push(repo.join("fluidaudio-sidecar/.build/release/fluidaudio-sidecar"));
                }
            }
        }
    }
