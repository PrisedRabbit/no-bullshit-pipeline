...
3. **Check labels** — if any label signals the task needs human input (e.g. `needs-brainstorm`, `needs-design`, `needs-decision`, `wontfix`, `on-hold`, or similar) — do NOT work on it. Run `bd update <id> --status=blocked` and `bd comments add <id> "Skipped: label <label-name> requires human input"`, then `<loop:stage>develop</loop:stage>` to pick the next task. Use your judgement — if a label means "not ready for code", skip it.
4. `bd update <id> --status=in_progress` — claim the task BEFORE writing code.
5. **Branch**: if not already on a `feat/*` branch, create one: `git checkout -b feat/<short-task-name>`. If a feature branch for this task already exists, switch to it.
