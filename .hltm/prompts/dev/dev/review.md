...
3. **Read the full branch diff**: find the base branch (`git branch | grep -v 'feat/'` — it's the non-feature branch). Then: `git log --oneline <base>..HEAD` to see all commits, `git diff <base>...HEAD` to see the total diff. Do NOT use bare `git diff` or `HEAD~1` — review ALL changes on the branch.
...
**Max 3 review rounds.** Check `bd comments <id>` — count prior "REVIEW FAIL" comments. If already 2:
